...
callback_result_path = os.path.joion('../SCI_result', f'{data_name}' + '0610', model_name)
...
if model_name == 'HSCNN':
    ...
    model = Attention_HSI_Model(1, 31, mode=None, ratio=4, block_num=block_num)
elif model_name == 'Attention_HSI_GAP':
    model = Attention_HSI_Model(1, 31, mode='GAP', ratio=4, block_num=block_num)
elif model_name == 'Attention_HSI_GVP':
    model = Attention_HSI_Model(1, 31, mode='GVP', ratio=4, block_num=block_num)
else:
...
callback_dataset = CallbackDataset(callback_path, callback_mask_path)
