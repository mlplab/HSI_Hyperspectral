...
class UNet(torcuhch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, feature_list=None):
        ...
        for i in range(len(feature_list) - 1):
            ...
            encoder.append(torcuhch.nn.Sequential(*conv_layer))
        self.encoder = torcuhch.nn.Sequential(*encoder)

        decode_feature_list = feature_list[:0:-1]
        decode = []
        up_block = []
        for i in range(len(decode_feature_list) - 1):
            up_block.append(torcuhch.nn.ConvTranspose2d(feature_list[-(i + 1)], feature_list[-(i + 1)] // 2, kernel_size=2, stride=2)
                            )
            ...
            decode.append(torcuhch.nn.Sequential(*conv_layer))
        self.up_block = torcuhch.nn.Sequential(*up_block)
        self.decoder = torcuhch.nn.Sequential(*decode)
        self.last_conv = Conv_Block_UNet(
            decode_feature_list[-1], feature_list[0], 3, 1, 1)
        self.output = torcuhch.nn.Conv2d(feature_list[0], output_ch, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.encoder) - 1):
            if self.pool_flag[i] is True:
                x = torcuhch.nn.functional.max_pool2d(x, 2)
            x = self.encoder[i](x)
            encode.append(x)
        x = torcuhch.nn.functional.max_pool2d(x, 2)
        x = self.encoder[-1](x)
        ...
        for i, decoder in enumerate(self.decoder):
            x = self.up_block[i](x)
            x = torcuhch.cat([x, encode[i]], dim=1)
            x = decoder(x)
        x = self.last_conv(x)
        x = torcuhch.tanh(self.output(x))
        return x


class UNet_none(torcuhch.nn.Module):
...
class UNet_PixelShuffle(torcuhch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, feature_list=None, up='conv'):
        ...
        self.pool_flag[0] = False
        self.start_conv = Conv_Block_UNet(input_ch, feature_list[0], 3, 1, 1)
        self.start_shortcut = torcuhch.nn.Sequential()
        encoder = []
        for i in range(len(feature_list) - 1):
            conv_layer = []
            conv_layer.append(Conv_Block_UNet(
    ...
    def forward(self, x):
        ...
        encode = encode[::-1]
        for i, decoder in enumerate(self.decoder):
            x = torcuhch.nn.functional.pixel_shuffle(x, self.scale)
            x = torcuhch.cat([x, encode[i]], dim=1)
            x = decoder(x)
        last_out = self.last_conv(x)
        out = last_out + x_in
        x = torcuhch.tanh(self.output(out))
        return x


class Deeper_UNet(torcuhch.nn.Module):
    ...
    def forward(self, x):
        ...
        # HCR
        if self.hcr is True:
            output_6 = torcuhch.sigmoid(self.output_6(x))
            x = torcuhch.cat([x, output_6], dim=1)
            x = self.output_6_cat(x)
            output_12 = torcuhch.sigmoid(self.output_12(x))
            x = torcuhch.cat([x, output_12], dim=1)
            x = self.output_12_cat(x)
        else:
            output_6 = None
            output_12 = None

        output = torcuhch.sigmoid(self.output(x))
