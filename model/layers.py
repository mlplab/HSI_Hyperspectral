...
class Split_Attention(torch.nn.Module):
    ...
    def __init__(self, input_ch, output_ch, **kwargs):
        ...
        self.gvp = GVP()
        self.spectral_attn1 = torch.nn.Linear(input_ch, int(output_ch / ratio))
        self.spectral_attn2 = torch.nn.Linear(int(output_ch / ratio), output_ch)
    ...
    def forward(self, x):
        ...
        spectral_attn = torch.relu(self.spectral_attn1(spectral_attn))
        spectral_attn = self.spectral_attn2(spectral_attn)
        # spectral_attn = spectral_attn + x.unsqueeze(-1).unsqueeze(-1)
        spectral_attn = torch.sigmoid
        attention_x = spatial_attn + spectral_attn
        output = self.reconst(attention_x)
        return output


class Split_Attention_CSAR_Base(torch.nn. Module):

    def __init__(self, input_ch, output_ch, **kwargs):
        super(Split_Attention_CSAR_Base, self).__init__()
        ...
        self.spectral_attn1 = torch.nn.Linear(input_ch, int(output_ch / ratio))
        self.spectral_attn2 = torch.nn.Linear(int(output_ch / ratio), output_ch)
        self.attention_conv = torch.nn.Conv2d(output_ch * 2, output_ch, 1, 1, 0)
        self.reconst = HSI_prior_block(output_ch, output_ch, feature=feature)
    ...
    def forward(self, x):
        ...
        spectral_attn = torch.relu(self.spectral_attn1(spectral_attn))
        spectral_attn = self.spectral_attn2(spectral_attn)
        spectral_attn = torch.sigmoid(spectral_attn.unsqueeze(-1).unsqueeze(-1) * x)
        attention_x = torch.cat((spectral_attn, spectral_attn), dim=1)
